...
class MainGUI:
    ...
    def __init__( self, window ):
        ...
        window.iconbitmap( default="C:\/Users\/dzajac\/Documents\/Chisen Projects\/Diaphragm Pump Tester\/transparent.ico" )
        ...
        stop_button = ttk.Button(Run_Frame, text="Stop Test", command=self.stop_handler)
        btn_spacer = ttk.Frame(Run_Frame, width=25)
        label_frame = ttk.Frame(Run_Frame, padding="5 0 0 0")
        freq_title = ttk.Label(Run_Frame, text="Frequency = ")
        self.freq_label = ttk.Label(label_frame, text=self.frequency)
        ...
        freq_title.grid(column=4, row=0)
        self.freq_label.pack()
        label_frame.grid(column=5, row=0)
    ...
    def run_handler( self ):
        if self.run_callback is not None:
            if self.Duty_cycle is not None:
                self.run_callback(100 - self.Duty_cycle)
            else:
                print("Error. No duty cycle was specified.")
        else:
    ...
    def freq_handler( self ):
        if self.freq_callback is not None:
            ...
            self.freq_label['text'] = self.frequency
        else:
